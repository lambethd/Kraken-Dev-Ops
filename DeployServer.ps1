...
if (-not $?)
{
    Write-Failure "Kraken-Server" "mvn clean install"
    exit 1
}
pscp -i C:\dev\kraken-key-pair-aws.ppk C:\dev\Kraken-Server\target\kraken-server-0.1.0.jar ec2-user@3.21.13.234:/var/kraken/
if (-not $?)
{
    Write-Failure "Kraken-Server" "deploy to server"
    exit 1
}
